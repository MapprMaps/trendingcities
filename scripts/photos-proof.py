...
def pick_best(city, country, cands):
    """OpenAI vision picks the best hero. Returns index (0-based) or -1."""
    key = os.environ["OPENAI_API_KEY"]
    content = [{"type": "text", "text":
        f"These are candidate photos for the HERO image of the city page for {city}, {country}. "
        f"Pick the ONE best: a wide, daylight skyline or cityscape that clearly and recognisably shows {city}, "
        f"magazine/editorial quality, NOT a close-up of a single building, NOT a map, NOT an indoor or product shot. "
        f"Reply with ONLY the number (1-{len(cands)}) of the best, or 0 if none are good."}]
    for c in cands:
        thumb = re.sub(r"w=\d+", "w=420", c["image_url"])
        content.append({"type": "image_url", "image_url": {"url": thumb, "detail": "low"}})
    body = {"model": "gpt-4o-mini", "messages": [{"role": "user", "content": content}], "max_tokens": 8, "temperature": 0}
    ...
    try:
        ...
        return n-1 if 1 <= n <= len(cands) else -1
    except Exception as e:
        print("   vision err:", str(e)[:80]); return 0  # fall back to top result
...
def main():
    ...
    for slug in SLUGS:
        ...
        idx = pick_best(rec["city"], rec["country"], cands)
        if idx < 0: print(f"  {slug}: vision found none good ({len(cands)} cands)"); continue
        ch = cands[idx]
